...
Note to Steve: you can skip the data cleaning as there are no issues with that - but I'll send you the data, so you can run the code if you want yourself. Go to line 590.
...
parties = read_csv("parties.csv", col_types = cols(
  ...
  mutate(prior_year_party_score = as.factor(party_score)) %>%
...
modelling_data = all_data %>%
  select(prior_year_cpi_score, prior_year_party_score, prior_year_gdp_per_capita, prior_year_resource_rents, e_participation_index, osi, hci, tti, prior_year_big_protest, prior_year_num_protests, prior_10_years_coups)
...
# 83 countries
# 7 years
all_data %>% select(country, year) %>%  table()
```

```{r}
plot(density(all_data$osi))
rug(all_data$osi)
```

```{r}
# transform outcome to logit scale
all_data %>%
...
form1 <- formula(osi_logit ~ prior_year_party_score + prior_year_cpi_score + prior_10_years_coups + prior_year_gdp_per_capita + hci + prior_year_resource_rents + prior_year_num_protests + tti + factor(year) + (1 | country))

model_full1 <- lmer(form1, data = all_data)

summary(model_full1)
```

```{r}
# linear mixed effects model with residual autocorrelation structure

library(nlme)

mod2 <- lme(fixed = osi_logit ~ prior_year_party_score + prior_year_cpi_score + prior_10_years_coups + 
                    prior_year_gdp_per_capita + hci + prior_year_resource_rents + prior_year_num_protests + tti + factor(year),
            random = ~ 1 | country,
...
```{r}
# function to squeeze [0, 1] to (0, 1)
# from Smithson & Verkuilen (2006)
squeeze <- function(x, N, s=0.5) {
  (x*(N-1) + s) / N
}

# squeeze craving scale to (0, 1)
all_data %>%
    mutate(osi_squeezed = squeeze(osi, n())) ->
all_data
```


```{r}
# frequentist beta regression model

library(glmmTMB)

form2 <- formula(osi_squeezed ~ prior_year_party_score + prior_year_cpi_score + prior_10_years_coups + prior_year_gdp_per_capita + hci + prior_year_resource_rents + prior_year_num_protests + tti + (1 | country) + (1 | year))

model_full2 <- glmmTMB(form2, family = beta_family(link = "logit"), data = all_data)
...
prior1 <- c(
  set_prior("student_t(3, 0, 5)", class = "b"), # df=3, mu=0, sigma=5
...
mod1 <- brm(
  ...
  cores = 4, chains = 4, init_r = 0.1,
  iter = 3e3, warmup = 1e3)

summary(mod1)
```

The full model here shows that nothing is significant except tti (which is technological infrastructure), cpi (corruption), gdp per capita, and resource rents. 

However, GDP per capita is significant in the "wrong" direction; it is implausible that more wealth is correlated with less e-government, even when controlling for related factors like education and technological infrastructure. Of course that could be a surprising finding, but I am skeptical that there isn't just something going on with the model.

```{r regression 1}
model_full = lm(osi ~ prior_year_party_score + prior_year_cpi_score + prior_10_years_coups + prior_year_gdp_per_capita + hci + prior_year_resource_rents + prior_year_num_protests + tti, data = modelling_data)
summary(model_full, conf.int = TRUE)
```

So, I tried looking at each variable's correlation with e-government individually, and they are all very significant. See attached chart in Word.

I though this might imply some kind of multicollinearity problem, but it looks like there aren't any strong correlations among the variables (here just the numeric ones)
...
modelling_data %>%
  ...
  #I'll figure out how to deal with missing data and include it better later
  cor(use = "complete.obs")
```
As you can see, there aren't any really strong correlations among the variables, so I don't understand why the significance is vanishing when I add them together in the same model. 

Here's a specific example of what really confuses me:

```{r}
protest_model = lm(osi ~ prior_year_num_protests, data = modelling_data)
summary(protest_model)
```
This is significant at the 0.1 level 

The correlation with tti, a control variable, is only 0.08.

```{r}
protest_model2 = lm(osi ~ prior_year_num_protests + tti, data = modelling_data)
summary(protest_model2)
```
Yet when I combine them, all of a sudden, the significance of protests is completely erased. Why would this happen when the protest and tti varibles are not strongly correlated?

I have similar problems with many of my variables. Here's another for a categorical variable:

```{r}
party_model = lm(osi ~ prior_year_party_score, data = modelling_data)
summary(party_model)
```
Now correlating with the control:
```{r}
party_model2 = lm(tti ~ prior_year_party_score, data = modelling_data)
summary(party_model2)
```
These are definitely correlated with each other, but tti hardly explains all the variability in the party variable because the R^2 value isn't that high, only 0.11.

But when I put them together:
```{r}
party_model3 = lm(osi ~ prior_year_party_score + tti, data = modelling_data)
summary(party_model3)
```
Party variable are no longer significant at all!
...
```{r model just novel}
model_novel = lm(osi ~ prior_year_party_score + prior_10_years_coups + prior_year_resource_rents + prior_year_num_protests, data = modelling_data)
tidy(model_novel, conf.int = TRUE)
summary(model_novel)
```

```{r}
numeric_only = modelling_data %>%
  select(prior_year_cpi_score, prior_year_gdp_per_capita, prior_year_resource_rents, e_participation_index, osi, hci, tti, prior_year_num_protests)
corr_df = data.frame(cor(numeric_only, use = "pairwise.complete.obs"))

ols_step_forward_p(model_full)

step_forward = lm(osi ~ tti + prior_year_cpi_score + prior_year_gdp_per_capita + prior_year_resource_rents + hci, data = modelling_data)
tidy(step_forward)
```
...
```{r}
model_demand = lm(osi ~ tti + prior_year_big_protest + hci + prior_year_resource_rents + prior_year_gdp_per_capita, data = modelling_data)
summary(model_demand)
```

```{r}
model_elite = lm(osi ~ tti + prior_10_years_coups + prior_year_party_score + prior_year_cpi, data = modelling_data)
summary(model_elite)
```
