...
modelling_data = all_data %>%
  select(year, prior_year_cpi_score, prior_year_party_score, prior_year_gdp_per_capita, prior_year_resource_rents, e_participation_index, osi, hci, tti, prior_year_big_protest, prior_year_num_protests, prior_10_years_coups)
...
```{r regression 1}
#Not a valid model because of autocorrelatin etc
...
```{r model just novel}
#Here I am just using most theoretically linked / previously unused variables
#This model is also invalid
model_novel_osi = lm(osi_logit ~ prior_year_party_score + prior_10_years_coups + prior_year_resource_rents + prior_year_big_protest + tti + as.character(year), data = all_data)
tidy(model_novel_osi, conf.int = TRUE)
summary(model_novel_osi)
```
```{r}
#Also invalid, same as above but for e-participation 
model_novel_e_participation = lm(e_participation_index_logit ~ prior_year_party_score + prior_10_years_coups + prior_year_resource_rents + prior_year_big_protest + tti + as.character(year), data = all_data)
tidy(model_novel_e_participation, conf.int = TRUE)
summary(model_novel_e_participation)
```


```{r}
#This is a starting point model from Steve in IQSS that is more valid because deals with panel data (not entirely sure what that means or what it does though under the hood?)
...
form1 <- formula(osi_logit ~ prior_year_party_score + prior_10_years_coups+ prior_year_resource_rents + prior_year_big_protest + tti + factor(year) + (1 | country))

mod1 <- lmer(form1, data = all_data)
...
```{r}
#Even more valid model by addressing autocorrelation 
#Not sure how many years back to do the autocorrelation though


mod2 <- lme(fixed = osi_logit ~ prior_year_party_score + prior_10_years_coups + prior_year_resource_rents + prior_year_big_protest + tti + factor(year),
            random = ~ 1 | country,
...
```{r}
#Same as above but with e-participation instead; basically the same

mod2 <- lme(fixed = e_participation_index_logit ~ prior_year_party_score + prior_10_years_coups + prior_year_big_protest + prior_year_resource_rents + tti + factor(year),
            random = ~ 1 | country,
            #Figure out how many levels of autoregression to go back  - supposed to make some kind of graph
            correlation = corARMA(form = ~ year | country, p=3, q=0),
            data = all_data,
...
#This seems to be the most interesting model so far in terms of seeing significant effects from my variables (I did check other interactions and nothing useful)

mod2 <- lme(fixed = osi_logit ~ prior_year_party_score + prior_year_big_protest + prior_10_years_coups * prior_year_resource_rents + tti + hci+ factor(year),
            random = ~ 1 | country,
            #Figure out how many levels of autoregression to go back  - supposed to make some kind of graph
            correlation = corARMA(form = ~ year | country, p=3, q=0),
            data = all_data,
